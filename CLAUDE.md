...
- **AI 唯一入口**：文本模型统一走 `lib/ai/provider.ts` 的 `generateText()`，经 CCSwitch 网关（`AI_BASE_URL`，本机 127.0.0.1:15721）转发到 `AI_MODEL`；文生图走 `generateImage()`，直连第三方 OpenAI 兼容 `/v1/images/generations`（`IMAGE_API_BASE_URL` / `IMAGE_API_KEY` / `IMAGE_MODEL`）。业务代码 / Agent 禁止直接 fetch 模型端点或写死 Provider。
