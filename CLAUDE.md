...
- **AI 唯一入口**：文本模型统一走 `lib/ai/provider.ts` 的 `generateText()`，直连 DeepSeek 官方 API（`EP_AI_BASE_URL` / `EP_AI_API_KEY` / `EP_AI_MODEL`）；文生图走 `generateImage()`，直连第三方 DashScope 原生 API（`EP_IMAGE_API_BASE_URL` / `EP_IMAGE_API_KEY` / `EP_IMAGE_MODEL`）。业务代码 / Agent 禁止直接 fetch 模型端点或写死 Provider。
- **联网调研**：Researcher Agent 走 `lib/ai/tavily.ts`（`EP_TAVILY_API_KEY`）做实时检索，产出入库 ResearchItem / Fact，无来源标 UNKNOWN。
