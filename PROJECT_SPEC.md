...
> 功能规格（是什么 / 工作流 / 数据模型）见 `README.md`；开发铁律见 `AI_DEVELOPMENT_RULES.md`；工程规范见 `CONTRIBUTING.md`。
...
6. 遵循 `CONTRIBUTING.md` 的目录 / 命名 / 序列化规范
7. 遵守 `AI_DEVELOPMENT_RULES.md` 铁律（尤其反幻觉、显式 max_tokens、统一 provider 入口）
...
| 海报导出 | 能从设计面板导出 PNG / PDF 文件，内容与 HTML 一致 |
